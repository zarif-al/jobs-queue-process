...
async fn main() {
    ...
    tokio::spawn(async {
        ...
        loop {
            ...
            match job {
                Some(job) => {
                    println!("Processing Thread 3 => Processing Job: {}", job.id);
                    sleep(PROCESSING_TIME).await;
                    work_queue
                        .complete(db, &job)
                        .await
                        .expect("Expected: Mark job as complete from processing thread 3.");
                    println!(
                        "Processing Thread 3 => Completed Processing Job: {}",
                        job.id
                    );
                }
                None => {}
